...
7. Run static validation. After reviewing state ownership, opt into runtime
...
8. Inspect existing brand assets. Create layered source artwork when necessary,
   package a modern `.icon`, add its project-relative path to `enmanner.json`,
   then run `preview-icon`. Open every Default, Dark, Clear, and Tinted
   rendition and confirm that the artwork is balanced and uncropped.
9. Run `build-app`. Verify the compiled icon, ownership marker, code signature,
   Finder/Dock appearance, and native launch behavior.
10. Report tracked source and framework files separately from ignored build
   products. Do not call the integration complete before step 9.
